...
export default function ProductCard({ product }: Props) {
    const { title, price, category, image, condition, slug } = product
    ...
    // prefer normalized src for relative /storage paths
    const src = image ? normalizeSrc(image) : null
        ...
        <article className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 hover:shadow-lg hover:border-indigo-100 transition-all duration-300 group">
            <Link href={`/products/${slug}`} className="block">
                ...
                <div className="w-full h-48 bg-gray-100 rounded-xl flex items-center justify-center overflow-hidden relative">
                    {src ? (
                        isAbsoluteUrl(src) ? (
                            // remote absolute url: use plain img to avoid next/image remote config problems
                            // eslint-disable-next-line @next/next/no-img-element
                            <img src={src} alt={title} className="object-cover w-full h-full group-hover:scale-105 transition-transform duration-300" />
                        ) : (
                            // local /storage/... path or internal asset -> use next/image for optimization
                            <Image
                                src={src}
                                alt={title}
                                width={320}
